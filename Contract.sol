...
contract Supplychain{
    ...
    function createShipment( uint256 _index, string memory _product_name, uint256 _quantity, address _sender1) public {
        shipments.push(Shipment("","",_sender1,0x0000000000000000000000000000000000000000,_index,_product_name,_quantity,ShipmentStatus.PENDING,PaymentStatus.UNPAID,StakeHolder.MANUFACTURER));
    }

    function startShipment(string memory _sender_name, string memory _receiver_name, address _sender, address _receiver, uint256 _index, uint8 _role, uint8 _pay) public{
        ...
        if(new_shipment.status == ShipmentStatus.PENDING){
            require(new_shipment.status == ShipmentStatus.PENDING, "Shipment Pending");
        }
        ...
        require(rolee == _role, "Shipment is currently assigned to different Stakeholder!");
        if(rolee==0){
    ...
    function completeShipment(uint256 _index,address _rec) public payable{
        Shipment storage new_shipment = shipments[_index];
        require(new_shipment.status == ShipmentStatus.IN_TRANSIT, "Shipment still pending or completed");
    ...
    function getShipment(uint256 _index) public view returns (string memory, string memory, address, address, uint256, string memory, uint256, ShipmentStatus, PaymentStatus, StakeHolder){
            Shipment storage new_shipment = shipments[_index];
            return (new_shipment.sender_name, new_shipment.receiver_name, new_shipment.sender, new_shipment.receiver, new_shipment.index, new_shipment.product_name, new_shipment.quantity, new_shipment.status, new_shipment.pay, new_shipment.role);
